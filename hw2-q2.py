...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.convBlock1 = nn.Sequential(
            nn.Conv2d(in_channels = 1, out_channels = 16, kernel_size = (3, 3), stride = 1, padding = 'same'), # 28x28x16         Verificar BIAS e IN_CHANNELS
            nn.ReLU(), # 28x28x16
            nn.MaxPool2d(kernel_size = (2,2), stride = 2) # 14x14x16
        )

        self.convBlock2 = nn.Sequential(
            nn.Conv2d(in_channels = 16, out_channels = 32, kernel_size = (3, 3), stride = 1, padding = 0),  #12x12x32             Verificar BIAS e IN_CHANNELS
            nn.ReLU(), #12x12x32
            nn.MaxPool2d(kernel_size = (2,2), stride = 2) # 6x6x32
        )
        ...
        self.network = nn.Sequential(
            ...
            self.convBlock2,
            nn.Flatten(),
            nn.Linear(in_features = 1152, out_features = 600), #Verificar BIAS
            nn.Dropout(0.3),
            nn.ReLU(),
            nn.Linear(in_features=600, out_features=120),  # Verificar BIAS
            nn.ReLU(),
